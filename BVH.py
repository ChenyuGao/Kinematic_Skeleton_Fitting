...
def load(filename, start=None, end=None, order=None, world=False):
    ...
    for line in f:
        ...
        if fmatch:
            ...
            jnum = len(parents)
            positions = offsets[np.newaxis].repeat(fnum, axis=0)    # (Frames, JointNum, 3)
    ...
    rotations = Quaternions.from_euler(np.radians(rotations), order=order, world=world)     # (Frames, JointNum, 4)
